...
export class SnapshotCache {
    ...
    ensureResolving(session) {
        const resolve = this.deps.resolveWorkspace;
        if (!resolve || this.workspacePaths.has(session))
            return undefined;
    ...
    async awaitResolved(session, timeoutMs = 1500) {
        const pending = this.ensureResolving(session);
        if (!pending)
            return;
        let timer;
        await Promise.race([pending, new Promise(r => { timer = setTimeout(r, timeoutMs); })]);
        clearTimeout(timer);
    }
    render(session) {
 ...
 */
export async function resolveWorkspacePath(ctx, cwd) {
    if (!cwd)
        return undefined;
    let canonical;
    try {
