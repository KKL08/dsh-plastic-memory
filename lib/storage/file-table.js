...
import { splitFrontmatter, decodeRecord, encodeRecord } from "./frontmatter.js";
import { WORKSPACE_MARKER, INDEX_FILE, GLOBAL_DIR, workspaceDirName, slugifyName, isReservedFileName } from "./paths.js";
...
export class FileTable {
    map = new Map();
    quarantine = [];
    wsDirs = new Map(); // workspacePath -> 绝对目录
    fingerprint = '';
    chain = Promise.resolve(); // 进程内互斥：写与重载串行化
    log;
    opts;
    constructor(opts) {
        this.opts = opts;
    ...
    quarantined() { return this.quarantine; }
    ...
    async loadInner() {
        ...
        for (const { dir, loc } of dirs) {
            ...
            try {
                ...
                this.log.warn(`[plastic-memory] 目录不可读，计空：${dir}`);
                continue;
            }
            for (const name of names) {
                if (/\.tmp-/.test(name)) {
                    ...
                    if (pidMatch && Number(pidMatch[1]) === process.pid) {
                        ...
                        try {
                            if (this.now() - (await stat(path)).mtimeMs > TMP_STALE_MS)
                                await rm(path, { force: true });
                        }
                        catch { /* 竞态消失 */ }
                    }
                ...
                if (!name.endsWith('.md') || name === INDEX_FILE)
                    continue;
        ...
        for (const slot of map.values()) {
            ...
            if (row)
                slot.record = { ...slot.record, recallCount: row.recallCount, lastRecalledAt: row.lastRecalledAt };
        }
    ...
    async computeFingerprint() {
        ...
        let dirNames = [];
        try {
            dirNames = await readdir(this.opts.root);
        }
        catch {
            return '';
        }
        for (const d of dirNames.sort()) {
            ...
            let names = [];
            try {
                names = (await readdir(dir)).filter(n => n.endsWith('.md') && n !== INDEX_FILE);
            }
            catch {
                continue;
            }
            for (const n of names.sort()) {
                try {
                    ...
                    parts.push(`${d}/${n}:${s.mtimeMs}:${s.size}`);
                }
                catch { /* 竞态删除 */ }
            }
        }
    ...
    async regenerateIndexes(map, wsDirs, onlyDirs) {
        const byDir = new Map();
        for (const slot of map.values()) {
            if (slot.record.status !== 'active')
                continue; // 索引只列 active
            const list = byDir.get(slot.dir) ?? [];
            list.push(slot.record);
            byDir.set(slot.dir, list);
        }
        ...
        const fmt = this.opts.formatIndexLine ?? ((r) => formatIndexLine(r, { passive: false }));
        // onlyDirs 给出时只再生这几个目录（byDir 仍取自全量 map，分组正确）；写路径只动到自己的目录，
        // 全量再生纯属浪费。缺省（加载路径）仍再生所有目录。
        const targetDirs = onlyDirs ?? new Set([join(this.opts.root, GLOBAL_DIR), ...wsDirs.values()]);
        for (const dir of targetDirs) {
            // 索引行序必须确定：Map 插入序取决于 readdir 序，跨设备不确定，多设备同步下会造成
            // 索引文件 churn。按 name 排序（不用 localeCompare——locale 依赖环境），name 相同用 id 决胜。
            const records = (byDir.get(dir) ?? []).slice().sort((a, b) => a.name < b.name ? -1 : a.name > b.name ? 1 : a.id < b.id ? -1 : a.id > b.id ? 1 : 0);
            const lines = records.map(fmt);
            const text = `# Memory Index\n\n${lines.join('\n')}${lines.length ? '\n' : ''}`;
            try {
                const prev = await readFile(join(dir, INDEX_FILE), 'utf8').catch(() => '');
                if (prev !== text)
                    await this.atomicWrite(join(dir, INDEX_FILE), text); // 内容不变不写：refresh 触发的重载不得空转写盘
            }
            catch { /* 索引是生成物，失败不致命 */ }
        }
    }
    async atomicWrite(filePath, text) {
        const tmp = `${filePath}.tmp-${process.pid}-${Math.random().toString(36).slice(2, 8)}`;
        await writeFile(tmp, text, 'utf8');
        await rename(tmp, filePath);
    }
    get(key) { return this.map.get(key)?.record; }
    entries() {
        return [...this.map.entries()].map(([k, s]) => [k, s.record]).values();
    }
    // —— 写侧 ——
    volatileEqual(a, b) {
    ...
    stableEqual(a, b) {
        const proj = (r) => { const { recallCount, lastRecalledAt, ...rest } = r; return rest; };
        return JSON.stringify(proj(a)) === JSON.stringify(proj(b));
    }
    async dirFor(record) {
        if (record.scope === 'global')
            return join(this.opts.root, GLOBAL_DIR);
    ...
    async filePathFor(record, dir) {
        ...
        for (const name of [`${base}.md`, `${base}-${idFrag}.md`]) {
            if (isReservedFileName(name))
                continue; // memory.md ≈ MEMORY.md，索引再生会覆盖它
            const path = join(dir, name);
    ...
    async persist(key, next) {
        ...
        if (stableChanged) {
            ...
            if (next.status === 'deleted')
                await this.sweepDeletedInner(this.map);
        ...
        if (stableChanged)
            this.fingerprint = await this.computeFingerprint(); // 自写后刷新基线，防假重载
    }
    async put(key, value) {
        await this.locked(() => {
