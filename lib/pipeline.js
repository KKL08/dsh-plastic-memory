import { CONFIDENCE_BY_SOURCE, memoryRecordSchema } from "./record-schema.js";
import { detectSecrets } from "./secrets.js";
import { extractEntities, textSimilarity, DUPLICATE_SIMILARITY_THRESHOLD } from "./text.js";
/**
...
export async function runSavePipeline(candidate, deps) {
    ...
    if (candidate.action === 'create' && !candidate.force) {
        ...
        const hits = peers.filter(p => {
            ...
            if (entities.length > 0)
                return extractEntities(peerText).some(e => entities.includes(e));
    ...
    const source = { sessionId: session.id, eventRange: [anchorStart, session.lastSeq], sourceMode: candidate.sourceMode };
    let record;
    if (candidate.action === 'update') {
    ...
    let updateSnapshotId;
    if (candidate.action === 'update' && target.scope === 'global' && deps.snapshots) {
        const snap = await deps.snapshots.capture({ operation: 'pre-update', description: `更新全局记忆 ${target.id} 前`, records: [target] }, now);
    ...
    const baseResult = candidate.action === 'update'
        ? { kind: 'updated', record: clean }
        : { kind: 'saved', record: clean };
