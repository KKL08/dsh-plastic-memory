 ...
 */
export function createPromoteTool(deps: PromoteToolDeps): ToolDefinition {
  return defineTool({
    name: 'memory_promote',
    ...
    parameters: {
      ids: { type: 'array', items: { type: 'string' }, required: true, description: '用户已经过 ask_user_question 确认要提升的候选记忆 id' },
    ...
    execute: (async (rawArgs: unknown) => {
      const args = rawArgs as { ids: string[]; target?: PromoteTarget }
      const target: PromoteTarget = args.target ?? deps.defaultTarget
      return executePromote({ confirmedIds: args.ids, target }, deps)
    }) as never,
  })
}
