...
const KNOWN_FILE_KEYS = new Set(FILE_KEY_ORDER);
const FENCE = '---\n';
const CLOSE = '\n---\n';
export function splitFrontmatter(raw) {
    if (!raw.startsWith(FENCE))
        throw new Error('缺少 frontmatter 围栏');
    const close = raw.indexOf(CLOSE, FENCE.length - 1);
    if (close < 0)
        throw new Error('frontmatter 围栏未闭合');
...
export function decodeRecord(parsed, loc) {
    const src = {};
    const extras = {};
    for (const [k, v] of Object.entries(parsed.data)) {
        if (KNOWN_FILE_KEYS.has(k))
            src[k] = v;
        else if (!NON_FILE_KEYS.has(k))
            extras[k] = v;
    ...
    for (const k of TIME_KEYS) {
        if (src[k] !== undefined) {
            ...
            if (!ISO_MS_RE.test(val))
                throw new Error(`时间戳必须是 UTC 毫秒 ISO 格式（toISOString）：${k}=${val}`);
            const ms = Date.parse(val);
            if (!Number.isFinite(ms))
                throw new Error(`时间戳无法解析：${k}=${val}`);
            src[k] = ms;
        }
    }
    const assembled = {
    ...
    if (!r.success)
        throw new Error(`schema 校验失败：${r.error.issues.map(i => `${i.path.join('.')}: ${i.message}`).join('；')}`);
    return { record: r.data, extras };
}
export function encodeRecord(record, extras) {
    const fm = {};
    for (const k of FILE_KEY_ORDER) {
        const v = record[k];
        if (v === undefined)
            continue; // 可选键缺省 = 键不存在
        fm[k] = TIME_KEYS.has(k) ? new Date(v).toISOString() : v;
    }
    for (const [k, v] of Object.entries(extras)) {
        if (!KNOWN_FILE_KEYS.has(k) && !NON_FILE_KEYS.has(k))
            fm[k] = v; // schema 键胜出
    }
