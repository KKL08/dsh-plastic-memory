import { FINDING_TYPES } from "../governance/schema.js";
import { runRuleScan } from "../governance/rule-scan.js";
import { scoreLayer, listWorkspaces, validateScopePaths, crossLayerConflicts, checkupRowBase } from "../governance/layer-health.js";
const DAY = 86_400_000;
/** tier 枚举 → 用户面中文文案（结构化 tier 字段仍保留 green/amber/red 供程序用）。 */
const TIER_LABEL = { green: '绿色', amber: '黄色', red: '红色' };
/** 把健康检查结果渲染成给模型看的一段文本。纯函数。 */
export function renderHealthResult(result) {
    ...
    const lines = [result.message];
    lines.push(`规则层扣分 ${breakdown.ruleLayer.penalty}（secret ${breakdown.ruleLayer.secret} / expired ${breakdown.ruleLayer.expired} / bloat ${breakdown.ruleLayer.bloat} / orphan ${breakdown.ruleLayer.orphan} / malformed ${breakdown.ruleLayer.malformed}）；`
 ...
 */
export async function executeHealth(rawArgs, deps, exec) {
    const args = rawArgs;
    const now = deps.now?.() ?? Date.now();
    const th = deps.thresholds;
    const overdueMs = th.pendingOverdueDays * DAY;
    if (args.scope === 'all') {
        const layers = [{ kind: 'global' }, ...listWorkspaces(deps.store).map(path => ({ kind: 'workspace', path }))];
    ...
    const layer = wsPath !== undefined ? { kind: 'workspace', path: wsPath } : { kind: 'global' };
    ...
    if (health.gate.secret) {
        ...
        const actions = [];
        if (overdue.length > 0)
            actions.push(`${overdue.length} 条冲突挂起超 ${th.pendingOverdueDays} 天，建议用 memory_confirm 裁决`);
        if (cachedAt === null)
            actions.push('尚未做过语义扫描，建议首次运行 memory_scan');
        else if (now - cachedAt > th.scanStaleDays * DAY)
            actions.push(`语义扫描已是 ${Math.floor((now - cachedAt) / DAY)} 天前，建议重新运行 memory_scan`);
        if (health.freshness.staleCount > 0)
            actions.push(`${health.freshness.staleCount} 条记忆超过衰退期，建议用 memory_confirm 确认或更新`);
        if (health.score < th.actionScoreThreshold)
            actions.push('存在待清理问题，建议运行 memory_scan 查看明细');
