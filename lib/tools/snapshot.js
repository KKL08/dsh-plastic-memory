 ...
 */
export async function executeSnapshotTool(rawArgs, deps) {
    const args = rawArgs;
    ...
    if (args.action === 'create') {
        ...
        const records = uniqueIds
            ? uniqueIds
                ...
                .filter((r) => r !== undefined && r.status !== 'deleted')
    ...
    // restore
    const { restored, skipped } = await deps.snapshots.restore(snap, args.memoryIds, r => deps.store.put(r), id => deps.store.get(id), args.overwriteChanged ?? false);
