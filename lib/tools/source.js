...
export function extractEventText(event) {
    ...
    if (!data)
        return null;
    const fromBlocks = (blocks) => (blocks ?? []).filter(b => b.type === 'text' && typeof b.text === 'string').map(b => b.text).join('');
 ...
 */
export async function executeSource(args, context, deps) {
    const record = deps.store.get(args.memoryId);
    if (!record || record.status === 'deleted')
        return { kind: 'not-found', memoryId: args.memoryId };
    ...
    if (!readEvent)
        return { kind: 'unavailable', memoryId: args.memoryId, reason: '会话查询服务未挂载' };
...
export function renderSourceResult(result, memoryName) {
    switch (result.kind) {
        case 'not-found':
