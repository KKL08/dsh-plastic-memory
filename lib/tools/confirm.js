 ...
 */
export async function executeConfirm(rawArgs, deps) {
    const args = rawArgs;
    ...
    if (vertical) {
        ...
        if (args.verdict === 'keep-right')
            toDelete = deps.store.get(entry.memoryIds[0]);
        if (args.verdict === 'keep-left')
            actions.push(`保留记忆 ${entry.memoryIds[0]}（用户确认 override ${entry.baselineRef}）`);
    }
    else {
        if (args.verdict === 'keep-left')
            toDelete = deps.store.get(entry.memoryIds[1]);
        if (args.verdict === 'keep-right')
            toDelete = deps.store.get(entry.memoryIds[0]);
    }
    if (toDelete && toDelete.status !== 'deleted') {
        ...
        if (args.verdict === 'keep-both')
            actions.push('两条并存（用户确认不冲突）');
