...
export function isAbort(err, signal) {
...
function clampBaseline(baseline) {
    if (baseline.length <= BASELINE_MAX_CHARS)
        return baseline;
...
没有问题输出 {"findings":[]}`;
export function buildSemanticPrompt(records, baseline) {
    const baselineSection = baseline
        ? `## 权威配置基线（AGENTS.md/CLAUDE.md）\n${clampBaseline(baseline)}`
        : '## 权威配置基线\n（无权威配置基线，跳过垂直冲突检测）';
    const memoryLines = records.map(r => `- id=${r.id} type=${r.type} scope=${r.scope}${r.workspacePath ? `(${r.workspacePath})` : ''}\n  content: ${r.content}\n  summary: ${r.summary}`);
    return {
...
export function parseSemanticFindings(raw, knownIds) {
...
export async function runSemanticScan(records, baseline, llm, signal) {
    ...
    if (first !== null)
        return { findings: first, failed: false, ...truncatedField };
    ...
    if (retry !== null)
        return { findings: retry, failed: false, ...truncatedField };
