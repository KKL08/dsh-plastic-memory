import { runRuleScan, buildMalformedFindings } from "../governance/rule-scan.js";
import { runSemanticScan, SEMANTIC_SCAN_MAX_RECORDS } from "../governance/semantic-scan.js";
import { scoreLayer, listWorkspaces, validateScopePaths, crossLayerConflicts, checkupRowBase, GLOBAL_CACHE_KEY } from "../governance/layer-health.js";
import { join, sep } from 'node:path';
import { workspaceDirName, GLOBAL_DIR } from "../storage/paths.js";
import { runCrossWsScan, findEntityDuplicates } from "../governance/cross-ws.js";
export function cacheKey(scope) {
...
export function renderScanResult(result) {
    ...
    if (result.kind === 'checkup') {
        ...
        const tierLabel = { green: '绿色', amber: '黄色', red: '红色' };
        ...
        if (result.promoteCandidates.total > 0) {
            ...
            for (const ws of result.promoteCandidates.byWorkspace) {
                for (const c of ws.items)
                    lines.push(`- [${c.id}] ${c.summary}（${ws.workspacePath}）`);
            }
        }
    ...
    if (result.promoteCandidates.items.length > 0) {
        ...
        if (result.promoteCandidates.more > 0)
            lines.push(`  …另有 ${result.promoteCandidates.more} 条提升候选未列出`);
        lines.push('提升前可用 memory_source 回查候选的出生语境，确认当时确实是跨项目通用的表述、不是项目绑定的顺口一句。');
    }
    return lines.join('\n');
}
/** 一次记录集的完整扫描（规则 + 语义 + 冲突挂账）。单层模式与体检的每层循环共用。 */
async function scanPass(deps, records, bucket, scopeLabel, quarantined, layers, llm, baseline, now, signal) {
    const notes = [];
    const findings = [];
    ...
    let semanticCachedAt = deps.cache.get(bucket)?.scannedAt ?? null;
    if (layers === 'semantic' || layers === 'full') {
        if (!llm) {
            ...
            const ordered = [...records].sort((a, b) => priorityOrder[deps.registry.get(a.type).governancePriority] - priorityOrder[deps.registry.get(b.type).governancePriority]
 ...
 */
export async function executeScan(rawArgs, deps, exec) {
    const args = rawArgs;
    ...
    if (checkup) {
        ...
        const notes = [];
        if (baseline === null)
            notes.push('未检测到 AGENTS.md 基线，垂直冲突检测未执行');
        const decisionItems = [];
        const seenDecisions = new Set();
        const collect = (pass) => {
            for (const item of pass.decisionItems) {
                if (seenDecisions.has(item.id))
                    continue; // 同一冲突可能在多层被复扫，呈现去重
        ...
        for (const ws of wsList) {
            ...
            if (baseline !== null && wsBaseline === null)
                baselineSkipped++;
        ...
        const layerRefs = [{ kind: 'global' }, ...wsList.map(path => ({ kind: 'workspace', path }))];
        ...
        if (byWorkspace.size >= 2) {
            ...
            if (llm && layers !== 'rule') {
                ...
                if (!cross.failed)
                    duplicates = { items: cross.items, via: 'semantic' };
                else {
    ...
    const records = deps.store.query({
        status: ['active'],
        ...(wsPath !== undefined
            ? { scope: { kind: 'visible', workspacePath: wsPath } }
            : { scope: { kind: 'global' } }),
    });
    ...
    if ((layers === 'semantic' || layers === 'full') && llm) {
        if (baseline === null)
            notes.push('未检测到 AGENTS.md 基线，垂直冲突检测未执行');
        else if (passBaseline === null)
            notes.push('基线随当前会话，扫描其他 workspace 时垂直冲突检测已跳过');
    }
