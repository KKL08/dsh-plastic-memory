...
import { dedupKey } from "./decisions.js";
import { FINDING_TYPES } from "./schema.js";
import { runRuleScan, buildMalformedFindings } from "./rule-scan.js";
import { computeHealth } from "./scoring.js";
...
export function layerCacheKey(layer) {
...
export function listWorkspaces(store) {
    ...
    for (const r of store.query({ status: ['active'] })) {
        if (r.scope === 'workspace' && r.workspacePath !== undefined)
            paths.add(r.workspacePath);
    }
    return [...paths].sort();
}
/**
 ...
 */
export function validateScopePaths(store, currentWs, requested) {
 ...
 */
export function scoreLayer(deps, layer, now) {
    ...
    const alive = (id) => deps.store.get(id)?.status === 'active';
    const semanticFindings = (cached?.findings ?? []).filter(f => {
        if (f.memoryIds.length === 0)
            return false;
        if (!f.memoryIds.every(alive) || !f.memoryIds.some(id => liveIds.has(id)))
            return false;
 ...
 */
export function crossLayerConflicts(store, decisions) {
    const byGlobalId = new Map();
    for (const e of decisions.list()) {
        const members = e.memoryIds.map(id => store.get(id)).filter((r) => r !== undefined);
        const globalSide = members.find(r => r.scope === 'global');
        const wsSide = members.find(r => r.scope === 'workspace' && r.workspacePath !== undefined);
        if (!globalSide || !wsSide)
            continue;
        const entry = byGlobalId.get(globalSide.id) ?? { workspaces: new Set(), decisionIds: [] };
...
export function issueSummary(result) {
    const parts = FINDING_TYPES
        .map(t => [t, result.health.counts[t]])
        .filter(([, n]) => n > 0)
        .map(([t, n]) => `${t} ${n}`);
    if (result.health.freshness.staleCount > 0)
        parts.push(`超期 ${result.health.freshness.staleCount}`);
    return parts.length > 0 ? parts.join('、') : '—';
}
/** 体检行里 scan 与 health 共有的字段。scan 另加 issues；health 另加 semanticCachedAt、promoteCandidates。 */
export function checkupRowBase(result) {
    return {
